...
|  --- | --- | --- | --- | --- | --- |
| `Intent` | [`CheckoutPaymentIntentEnum`](../../doc/models/checkout-payment-intent-enum.md) | Required | The intent to either capture payment immediately or authorize a payment for an order after order creation. | CheckoutPaymentIntentEnum getIntent() | setIntent(CheckoutPaymentIntentEnum intent) |
| `Payer` | [`Customer`](../../doc/models/customer.md) | Optional | DEPRECATED. The customer is also known as the payer. The Payer object was intended to only be used with the `payment_source.paypal` object. In order to make this design more clear, the details in the `payer` object are now available under `payment_source.paypal`. Please use `payment_source.paypal`. | Customer getPayer() | setPayer(Customer payer) |
